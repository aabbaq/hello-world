...
- 不存在GC，但会OOM
- 可能出现两种异常：
  1. **StackOverflowError**：线程申请的栈空间超过了虚拟机栈允许的最大容量，可以理解为超出了栈的深度（常出现在无限递归的情况中）；
  2. **OutOfMemoryError**：如果栈空间可以动态扩展，但内存中没有空间可以被扩展时，即无法申请到内存时；
- 可以使用 `-XssSIZE` （**注意，紧跟着设置的大小**）：设定虚拟机栈的大小，默认以字节作为单位，可以添加 `k m g` 来设置不同级别的的栈大小
...
类似于虚拟机栈，但是调用的方法为本地方法，如C、C++、或者汇编的方法
...
存储数组和对象的实例，堆空间**可以在物理上不连续**，但在逻辑上是连续的。对堆空间进行分代，或者说是区域划分主要是为了优化GC的性能，该区域是垃圾回收的主要区域
...
一种规范，实现有可能不同，该区域用于存储已被虚拟机加载的：

1. **类型信息**
2. **常量**
3. **静态变量**
4. **即时编译器**（JIT）编译后的**代码缓存**等

特点为：

- 大小可**动态调整**，JDK 8后不属于JVM，而存在于本地内存（实现为元空间）中，和堆相似，**动态扩展失败**后会抛出OOM异常
...
- 触发GC的条件，达到高水平线，即 `MetaspaceSize` 的大小就会进行GC，然后根据GC释放的空间多少决定元空间的动态调整
  - 回收的主要内容是：
    ...
    - **废弃的类，完全不会再用到的类**，条件比较苛刻，包含：
  
      1. **所有的类实例**需要都被回收，堆中已不存在任何该类实例
      2. 加载该类的**类加载器**（ClassLoader ）也已被回收
      3. 该类**对应的 `java.lang.Class` 对象未被引用**（无法通过反射访问类方法）
  
      即使上述三个条件被满足，也**不一定**会被回收

> Q：为什么要移除永久代而实现元空间，即为什么要讲方法区从JVM内存中转移至空间更大的本地内存中？
>
> A：HotSpot 虚拟机把它当成**永久代**来进行垃圾回收。但很难确定永久代的大小，因为它受到很多因素影响，并且每次 Full GC 之后永久代的大小都会改变，所以经常会抛出 OutOfMemoryError 异常。

#### 运行时常量池

补充内容：**运行时常量池**属于方法区的一部分，Class 文件中的**常量池**（编译器生成的字面量和符号引用）会在类加载后被放入这个区域。

>  `intern` 方法可以在运行时动态生成常量放入常量池，存疑，目前不清楚运行时常量池和运行时字符串常量池的区别
...
- **引用计数法**
  ...
  为对象添加引用计数器，对象每存在一个引用，为计数器+1，失去引用则-1。引用计数器为0的对象将会被回收

  - 实现**简单**，效率高，回收不会有延迟
  - 需要**额外空间**存储计数器，用于计算对象的（被）引用次数
  - 更新计数器需要额外开销
  - **不能处理循环引用**的情况
...
- 标记-清除法
  ...
  在标记阶段，检查每个对象是否为存活对象，若存活则在**对象头**打上标记；在清除阶段，回收对象并判断是否与前一块空闲空间相连，若为连续空间，则合并两个分块

  - **简单**，但是效率**不稳定**，需要**遍历整个空间**以获得标记的非垃圾对象
  - 适合**存活对象多**，**回收对象少**的场景
  - 会产生内存碎片
  - 需要维护一个**空闲列表**，通过空闲列表来判断内存是否可用，空闲链表是一个单链表结构，之后分配空间只需要遍历该链表即可

- 标记-复制法（复制法）
  
  在主流的回收算法中，将对区域新生代部分分为Eden以及S0，S1区域，复制的时候会将Eden区域的满足要求的存活对象以及某一个S区的存活对象复制进另一个S区域
  
  - 效率高，只需要在可达性分析时就可以将非垃圾对象复制
  - 适合**存活对象少**的场景
  - 需要**额外的内存空间**（2倍）
  - 需要**担保机制**，为了保证空间够用，但空间不至于太大，需要控制S1与S2区的大小，若此时空间仍然不够用，需要从其他区域（例如老年代）借一些空间，这被称为担保机制
  - 在垃圾不多的时候，效率不高（复制大量的非垃圾对象）
  
- 标记-整理法（标记压缩法）
  
  标记的存活对象全部移动到区域的一端（头部），后续的区域直接清空
...
- **强引用**：最常用的引用，如果存在强引用的关系，就不会被垃圾回收
  - 常见的引用类型，也是默认的引用类型
  - 是造成Java内存泄漏的主要原因
- **软引用**：即将发生内存溢出时（即内存不够的情况下），才会将软引用对象列入回收目标中
  - 主要目的是为了实现高速缓存
- **弱引用**：弱引用的对象在下一次垃圾收集器工作时，会被回收，即一定会被回收，并且确定了回收的时间点
  - 可指定引用队列跟踪其情况
